...
export function ReceiptResultModal({ isOpen, onClose, initialData, onSave }: ReceiptResultModalProps) {
  const [date, setDate] = useState(new Date(2026, 0, 31)); // January 31, 2026
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [items, setItems] = useState<ExpenseItem[]>(
    initialData?.items || [{ id: "1", name: "", amount: "", category: "未分類" }]
  );

  useEffect(() => {
    if (isOpen && initialData) {
      // Parse initialData.date if needed
                ...
                <div className="size-[24px] flex items-center justify-center">
                  ...
                  >
                    <g>
                      ...
                      <g mask="url(#mask0_receipt_result)">
                ...
                <div className="flex flex-col gap-[8px]">
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder="2026年1月31日 (土)"
                    />
                    <button
                      onClick={() => setShowDatePicker(true)}
                      className="absolute right-[16px] top-1/2 -translate-y-1/2 size-[24px] rounded-full bg-[#f3f4f6] hover:bg-[#e2e9f2] transition-colors z-10"
                      aria-label="日付を選択"
                    >
                      <Calendar className="size-[14px] text-[#7C7A78]" />
                    </button>
                  </div>
                </div>
